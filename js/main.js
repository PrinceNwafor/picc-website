...
(function () {
  ...
  if (nav) {
    const handleScroll = () => {
      // Pages that aren't meant to have a transparent hero nav stay solid always.
  ...
  function showToast(message, type = 'success') {
    ...
    Object.assign(toast.style, {
      ...
      background: type === 'success' ? '#8B2E68' : '#C6A23C',
      color: 'white',
      padding: '14px 24px',
      borderRadius: '40px',
  ...
  /* --------------------------------------------------------------------------
     16. HERO VIDEO — sound on/off toggle
     -------------------------------------------------------------------------- */
  const heroVideo = document.querySelector('.hero__video');
  const soundBtn = document.querySelector('.hero__sound');
  if (heroVideo && soundBtn) {
    const iconOn = soundBtn.querySelector('[data-icon="on"]');
    const iconOff = soundBtn.querySelector('[data-icon="off"]');
    const sync = () => {
  ...
  if (galleryItems.length) {
    ...
    const closeBox = () => { box.classList.remove('lightbox--open'); document.body.style.overflow = ''; };
